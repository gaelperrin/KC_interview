...
class VolumeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train:
            ...
            return [
                torch.tensor(volume.values, dtype=torch.float32), 
                torch.tensor(missing_intraday, dtype=torch.float32), 
                torch.tensor(total_volume, dtype=torch.float32),
                torch.tensor(period_k, dtype=torch.float32),
                torch.tensor(date_d, dtype=torch.float32)
                ]
        else:
...
def train(data_loader, model, loss_fn, optimizer):
    model.train()
    loss_list = []
    for (volume, missing_intraday, total_volume, period_k, date_d) in train_loader:
...
def test(data_loader, model, loss_fn):
    model.eval()
    loss_list = []
    for (volume, missing_intraday, total_volume, period_k, date_d) in test_loader:
        volume = volume.to(device)
        missing_intraday = missing_intraday.to(device)
        total_volume = total_volume.to(device)
        input = torch.cat([volume, missing_intraday.unsqueeze(-1)], dim=1)
...
for epoch in range(num_epochs):
    ...
    average_loss_test = test(train_loader, model, criterion)
